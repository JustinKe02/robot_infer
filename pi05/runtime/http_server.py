...
class PolicyRequestHandler(BaseHTTPRequestHandler):
    ...
    def _authenticate(self) -> bool:
        ...
        valid = (
            bool(separator) and scheme.lower() == "bearer" and hmac.compare_digest(supplied.strip(), expected)
        )
        if valid:
            return True
        self._send_json(
    ...
    def _read_request_body(self) -> bytes:
        ...
        if length > self.server.max_request_bytes:
            raise ValueError(f"Request body is too large: {length} > {self.server.max_request_bytes}")
...
def validate_bind_security(host: str, auth_token: str | None) -> str | None:
    ...
    if not is_loopback_host(host) and token is None:
        raise ValueError(f"Refusing non-loopback bind host={host!r} without an authentication token")
    return token
