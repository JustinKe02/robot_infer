...
DEFAULT_MAX_CAMERA_STATE_RECEIVE_SKEW_MS = 250.0
DEFAULT_CALIBRATION_DIR = Path(__file__).resolve().parents[1] / "run_state" / "calibration"
...
def build_zmq_camera_configs(
    *,
    ...
    for camera_name in camera_names(camera_profile):
...
def build_robot_config(
    *,
    ...
    max_camera_state_receive_skew_ms: float = DEFAULT_MAX_CAMERA_STATE_RECEIVE_SKEW_MS,
    ...
    return JZRobotPinTimedConfig(
        ...
        reject_reused_camera_frames=reject_reused_camera_frames,
...
def build_dataset_artifacts(
    ...
    *,
    ...
    validate_live_dataset_features(dataset_features, camera_profile=camera_profile)
    return dataset_features, robot_action_processor, robot_observation_processor


def validate_live_dataset_features(
    dataset_features: Mapping[str, Mapping[str, Any]],
    *,
    camera_profile: str = DEFAULT_CAMERA_PROFILE,
) -> None:
    ...
    expected_camera_features = camera_dataset_shapes(camera_profile)
    ...
    if actual_camera_features != expected_camera_features:
        raise ValueError(
            f"Live camera features differ from the trained {camera_profile!r} contract; "
