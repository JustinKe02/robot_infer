...
class RemotePolicyClient:
    ...
    def infer(self, request: InferenceRequest) -> InferenceResponse:
        ...
        if payload.request_id != request.request_id:
            raise RuntimeError(f"Response request_id mismatch: {payload.request_id} != {request.request_id}")
    ...
    def _read_limited(self, response: Any) -> bytes:
        ...
        if raw_length is not None:
            ...
            if length > self.max_response_bytes:
                raise ValueError(f"Server response is too large: {length} > {self.max_response_bytes}")
        ...
        if raw_length is not None and len(body) != int(raw_length):
            raise ValueError(f"Server response ended early: expected {raw_length}, got {len(body)}")
        return body
