...
POLICY_PATH = REPO_ROOT / "tk_infer/pi05/checkpoints/010600/pretrained_model"
...
def verify_checkpoint() -> dict[str, Any]:
    ...
    if actual_files != expected_files:
        raise RuntimeError(
            "step_010600 file set differs from the audited source; "
    ...
    if mismatched_hashes:
        raise RuntimeError(f"step_010600 SHA-256 mismatch: {mismatched_hashes}")
    ...
    if mismatched_metadata:
        raise RuntimeError(f"step_010600 metadata mismatch: {mismatched_metadata}")
    return {
        "ok": True,
