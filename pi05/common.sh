...
PI05_TASK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TK_INFER_DIR="$(cd "${PI05_TASK_DIR}/.." && pwd)"
REPO_ROOT="$(cd "${PI05_TASK_DIR}/../.." && pwd)"
...
# Server startup requires an explicit completed pretrained_model directory.
# Client-only and config-only commands do not need a local policy.
...
RUNTIME_DIR="${RUNTIME_DIR:-${PI05_TASK_DIR}/run_state}"
CACHE_DIR="${CACHE_DIR:-${RUNTIME_DIR}/cache}"
LOG_ROOT="${LOG_ROOT:-${PI05_TASK_DIR}/logs}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PI05_TASK_DIR}/outputs}"

rtc_die() {
  echo "[tk_infer/pi05] ERROR: $*" >&2
  exit 2
}
...
rtc_require_local_write_path() {
  ...
  case "${resolved}" in
    "${PI05_TASK_DIR}"|"${PI05_TASK_DIR}"/*)
      ;;
    *)
      rtc_die "${name} must stay inside ${PI05_TASK_DIR}, got ${resolved}"
      ;;
  esac
}
...
rtc_run_or_print() {
  ...
  if [[ "${print_only}" == "true" ]]; then
    echo "[tk_infer/pi05] PRINT_COMMAND_ONLY=true; nothing was started"
    return 0
  fi
  ...
  echo "[tk_infer/pi05] log=${log_path}"
  (
    cd "${RUNTIME_DIR}"
    "${command[@]}"
  ) 2>&1 | tee "${log_path}"
