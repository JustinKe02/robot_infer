...
from tk_infer.pi05.runtime.camera_profiles import camera_feature_keys
from tk_infer.pi05.runtime.checkpoint import (
...
def _make_checkpoint(tmp_path: Path) -> SyntheticCheckpoint:
    ...
    preprocessor = {
        "steps": [
            {
                "class": (
                    "lerobot.robots.jz_robot_pin_timed.training_schema.JZPinRaw18ToTraining16ProcessorStep"
                ),
...
def _mutate_embedded_schemas(
    ...
    for filename in ("policy_preprocessor.json", "policy_postprocessor.json"):
        ...
        schema_steps = [step for step in config["steps"] if "schema" in step.get("config", {})]
...
def test_inspect_checkpoint_accepts_complete_audited_synthetic_checkpoint(
    ...
    assert metadata.camera_profile == "three_camera"
...
def test_inspect_checkpoint_accepts_head_right_camera_profile(
    synthetic_checkpoint: SyntheticCheckpoint,
) -> None:
    config = synthetic_checkpoint.read_json("config.json")
    del config["input_features"]["observation.images.camera_left"]
    synthetic_checkpoint.write_json("config.json", config)

    metadata, _schema = _inspect(synthetic_checkpoint)

    assert metadata.camera_profile == "head_right"
    assert metadata.camera_keys == camera_feature_keys("head_right")
    assert set(metadata.camera_shapes) == set(camera_feature_keys("head_right"))
