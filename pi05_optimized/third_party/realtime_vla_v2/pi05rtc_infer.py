...
# Vendored from dexmal/realtime-vla-v2 at a36d02a7b241de1129af2048e749de58f95ead9c.
...
def build_rtc_token_condition_bases(
    weights: dict[str, torch.Tensor],
) -> tuple[torch.Tensor, torch.Tensor]:
    """Reproduce the local training-time token-flow and clean-prefix embeddings."""
    device = weights["rtc_token_time_mlp_in_w"].device
    embedding_dim = int(weights["rtc_token_time_mlp_in_w"].shape[0])
    time_zero = _posemb_sincos_torch(
        torch.zeros(1, dtype=torch.float32, device=device),
        embedding_dim=embedding_dim,
    )

    def project(raw_time: torch.Tensor) -> torch.Tensor:
        hidden = _swish(
            torch.matmul(raw_time.float(), weights["rtc_token_time_mlp_in_w"].float())
            + weights["rtc_token_time_mlp_in_b"].float()
        )
        return (
            torch.matmul(hidden, weights["rtc_token_time_mlp_out_w"].float())
            + weights["rtc_token_time_mlp_out_b"].float()
        )

    suffix = project(weights["decoder_time_embeds"]) + weights["rtc_prefix_embedding"][0].float()
    prefix = project(time_zero)[0] + weights["rtc_prefix_embedding"][1].float()
    return suffix.to(torch.bfloat16), prefix.to(torch.bfloat16)
...
def transformer_decoder_rtc(weights, buffers, encoder_seq_len, num_steps=10):
    for step in range(num_steps):
        ...
        buffers["decoder_x"].add_(buffers["decoder_rtc_token_condition"][step])
...
class Pi05RTCInference:
    def __init__(
        ...
        self.weights = {
            ...
            "rtc_token_time_mlp_in_w":            torch.empty(1024, 1024,             dtype = torch.bfloat16, device = "cuda"),
            "rtc_token_time_mlp_in_b":            torch.empty(1024,                   dtype = torch.bfloat16, device = "cuda"),
            "rtc_token_time_mlp_out_w":           torch.empty(1024, 1024,             dtype = torch.bfloat16, device = "cuda"),
            "rtc_token_time_mlp_out_b":           torch.empty(1024,                   dtype = torch.bfloat16, device = "cuda"),
            "rtc_prefix_embedding":               torch.empty(2, 1024,                dtype = torch.bfloat16, device = "cuda"),
        ...
        self.buffers = {
            ...
            "decoder_rtc_token_condition":        torch.empty((10, decoder_seq_len, 1024),          dtype = torch.bfloat16, device = "cuda"),
        ...
        self._update_runtime_adarms_mods()
        self._build_rtc_token_condition_bases()
        self._update_runtime_rtc_token_conditions(prefill_len=0)
    ...
    def _update_runtime_adarms_mods(self) -> None:
    ...
    def _build_rtc_token_condition_bases(self) -> None:
        self._rtc_suffix_condition, self._rtc_prefix_condition = build_rtc_token_condition_bases(
            self.weights
        )

    def _update_runtime_rtc_token_conditions(self, prefill_len: int) -> None:
        self.buffers["decoder_rtc_token_condition"][:] = self._rtc_suffix_condition[:, None, :]
        if prefill_len > 0:
            self.buffers["decoder_rtc_token_condition"][:, :prefill_len, :] = self._rtc_prefix_condition[
                None, None, :
            ]
    ...
    def forward(
        ...
        self._update_runtime_rtc_token_conditions(prefill_len)
